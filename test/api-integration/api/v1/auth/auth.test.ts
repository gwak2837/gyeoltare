...
describe("Better Auth 통합 테스트", () => {
  ...
  test("로그아웃하면 Better Auth 세션이 제거된다", async () => {
    const client = environment.createClient();
    const user = createEmailPasswordUser();
  ...
  test("중복 회원가입은 실패 경로를 명확하게 드러낸다", async () => {
    const client = environment.createClient();
    const user = createEmailPasswordUser();

    const firstSignUp = await client.post("/api/v1/auth/sign-up/email", { body: user });
    const duplicateSignUp = await client.post("/api/v1/auth/sign-up/email", { body: user });

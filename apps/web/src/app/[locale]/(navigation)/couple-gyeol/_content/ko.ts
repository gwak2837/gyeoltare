...
export const rarityContent = {
  ...
  grades: {
    1: {
      description:
        "결타래 모델에서 네 가지 축이 모두 선명하게 맞물린 조합이에요. 둘만의 기준이 강하고, 함께 있을 때 관계의 결이 또렷하게 드러나요.",
      label: "1등급",
      mountainLabel: "선명한 결",
    },
    2: {
  ...
  results: {
    ...
    rare: {
      ...
      summary:
        "우리의 결은 결타래 모델 기준으로 꽤 선명한 조합이에요. 남들이 보면 평범한 순간도, 둘 사이에서는 바로 통하는 기준처럼 작동해요.",
    },
    reconnect: {
  ...
  ui: {
    ...
    modelStepGradeBody: "선택 조합을 결타래 모델 기준으로 읽고, 1등급부터 7등급까지의 위치를 보여줘요.",
    ...
    modelNotice: "정답을 맞히는 검사가 아니라, 답변 조합을 결타래 모델 기준으로 읽은 결과예요.",
    nextButton: "다음 문항",
